...
class ClassificationModule(L.LightningModule):
    ...
    def __init__(self, cfg, model, multitask: bool = False):
        ...
        if self.multitask:
            self.train_segmentation_metrics = MetricCollection(
                [JaccardIndex(task="binary", num_classes=2, threshold=0.5)],
                prefix="train/segmentation/",
            )
            self.val_segmentation_metrics = MetricCollection(
                [JaccardIndex(task="binary", num_classes=2, threshold=0.5)],
                prefix="val/segmentation/",
            )
    ...
    def training_step(self, batch, batch_idx):
        if self.multitask:
            ...
            loss = self.classification_loss(
                class_logits, labels_ohe
            ) + 0.50 * self.segmentation_loss(seg_logits, mask)

            cls_metrics = self.train_classification_metrics(class_logits, y.int())
            seg_metrics = self.train_segmentation_metrics(seg_logits, mask.int())
    ...
    def validation_step(self, batch, batch_idx):
        if self.multitask:
            ...
            self.val_classification_metrics.update(class_logits, y.int())
            self.val_segmentation_metrics.update(seg_pred, mask.int())
    ...
    def on_validation_epoch_end(self):
        ...
        confmat = cls_metrics.pop(
            "val/classification/MulticlassConfusionMatrix"
        )  # shape: (C, C)
        if not self.multitask:
            self.log_dict(cls_metrics, prog_bar=True, on_step=False, on_epoch=True)
        else:
            self.log_dict(
                {**cls_metrics, **seg_metrics},
                prog_bar=True,
                on_step=False,
                on_epoch=True,
            )
